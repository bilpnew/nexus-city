...
const MissionView: React.FC<MissionViewProps> = ({ missions, characters, onAccept, onReplay, onResolveEvent, onShowTutorial, onGenerateMission, isGenerating }) => {
  ...
  const getDifficultyColor = (diff: Mission['difficulty']) => {
    switch (diff) {
      ...
      case 'Legendary': return 'text-purple-500 bg-purple-500/10 border border-purple-500/30';
                  ...
                  <>
                    ...
                    <div className="bg-zinc-950/80 p-4 rounded-xl border border-white/5 h-20 overflow-y-auto mb-4 font-mono text-[9px] text-zinc-400 space-y-1.5 scrollbar-hide shadow-inner">
                      {(mission.tacticalLogs || []).map((log, i) => (
                        <div key={i} className={i === 0 ? "text-cyan-400" : "opacity-60"}>
